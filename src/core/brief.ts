...
export type AttentionKind = "task_overdue" | "task_due_soon" | "waiting_review" | "account_at_risk" | "account_quiet" | "account_ungrounded" | "deal_unattended";
...
export function buildBrief(workspace: Workspace, now: Date = new Date()): Brief {
  ...
  const active = (id?: string) => !id || !byId.get(id)?.archivedAt;
  const openTasks = workspace.tasks.filter((task) => task.status === "open" && active(task.accountId));
  const openDeals = workspace.deals.filter((deal) => isOpenDeal(deal.stage) && active(deal.accountId));
  ...
  for (const task of workspace.tasks.filter(task => task.status === "waiting" && active(task.accountId))) {
    if (new Date(task.due).getTime() <= t) taskItem(task, "waiting_review", 4, `Review the waiting trigger for "${task.title}": ${task.reason}. This is not an outreach instruction.`);
  }
  ...
  for (const account of workspace.accounts) {
    ...
    if (notes.length === 0) {
      ...
      const quietFor = account.lastTouch ? t - new Date(account.lastTouch).getTime() : Infinity;
      if (quietFor > QUIET_DAYS * DAY_MS) accountItem(account, "account_quiet", 4, account.lastTouch ? `${account.name} has had no recorded touch in ${plural(days(quietFor), "day")}. Verify context; silence is not an outreach instruction.` : `${account.name} has no verified interaction date. Research capture is not contact.`);
    }
  }
  ...
  attention.sort((a, b) => a.rank - b.rank || a.recordId.localeCompare(b.recordId));
